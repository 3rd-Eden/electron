...
if (process.guestInstanceId == null) {
  ...
  window.close = function () {
    ipcRenderer.sendSync('ELECTRON_BROWSER_WINDOW_WINDOW_CLOSE')
  }
}
...
window.alert = function (message, title) {
  ipcRenderer.sendSync('ELECTRON_DIALOG_WINDOW_ALERT', message, title)
}

window.confirm = function (message, title) {
  return ipcRenderer.sendSync('ELECTRON_DIALOG_WINDOW_CONFIRM', message, title)
}

// But we do not support prompt().
window.prompt = function prompt () {
  const error = new Error('prompt() is and will not be supported.')
  // Remove stack and recapture without current frame
  delete error.stack
  captureStackTrace(error, prompt)
  throw error
}
